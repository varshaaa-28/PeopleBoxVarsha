...
select * from responses left join employees on responses.employee_id=employees.id;

# 1)
select round(avg(score),2) as Average_Score,driver_name from responses group by driver_name having count(score)>2;

#2)
select round(avg(score),2) as Average_Score, driver_name, department from responses left join employees on responses.employee_id=employees.id group by department,driver_name having count(score)>2;

#3)
select round(avg(score),2) as Average_Score,location,driver_name from responses left join employees on responses.employee_id=employees.id group by location,driver_name having count(score)>2;

#4)
select round(avg(score),2)as Average_Score,gender,driver_name from responses left join employees on responses.employee_id=employees.id group by gender,driver_name having count(score)>2;

#5)
select round(avg(score),2) as Average_Score,case when (age<30 and age>=20) then "20-30" when (age>=30 and age<40) then "30-40" when (age>=40 and age<=50) then "40-50"  else "rest" end as age_group
 from responses left join employees on responses.employee_id=employees.id group by age_group having count(score)>2 order by age_group ;

#6)
select round(avg(score),2) as Average_Score ,name from responses left join employees on employees.id=responses.employee_id group by name having count(score)>2;
